...
def obtener_link_interactivo(base_url):
    try:
        ...
        initial_url = base_url  # Guardamos la URL inicial
        ...
        while continue_navigation:
            ...
            if not contenedor:
                ...
                continue_download = input("¿Desea descargar más bases de datos? (s/n): ").strip().lower()
                if continue_download != 's':
                    return selected_files
                # Reiniciar a la URL inicial si el usuario quiere continuar
                current_url = initial_url
                break

            # Buscar enlaces dentro del contenedor específico
            links = []
            excel_links = []
            print("\nOpciones disponibles:")
            for idx, link in enumerate(contenedor.find_all('a', href=True), start=1):
                text = link.text.strip()
                href = link['href']
                if text:
                    absolute_url = urljoin(current_url, href)
                    links.append((text, absolute_url))
                    print(f"[{idx}] {text} -> {absolute_url}")

                    if absolute_url.lower().endswith(('.xls', '.xlsx')):
                        excel_links.append((text, absolute_url))

            while True:
                if excel_links:
                    print("\nArchivos Excel disponibles:")
                    for idx, (text, url) in enumerate(excel_links, start=1):
                        print(f"[{idx}] {text}")

                    print("\nSeleccione archivos Excel (números separados por comas, o 'q' para terminar):")
                    user_input = input(">> ").strip()

                    if user_input.lower() == 'q':
                        print("\n¿Desea buscar más bases de datos? (s/n):")
                        continue_select = input(">> ").strip().lower()
                        if continue_select != 's':
                            return selected_files
                        break
                    ...
                    try:
                        ...
                        for choice in choices:
                            if 1 <= choice <= len(excel_links):
                                selected_text, selected_url = excel_links[choice - 1]
                                file_name = os.path.basename(selected_url)
                                file_path = os.path.join(database_folder, file_name)

                                print(f"\nDescargando: {file_name}")
                                try:
                                    file_response = requests.get(selected_url)
                        ...
                        print("\n¿Desea descargar más bases de datos? (s/n):")
                        continue_download = input(">> ").strip().lower()
                        if continue_download != 's':
                            return selected_files
                        # Reiniciar a la URL inicial si el usuario quiere continuar
                        current_url = initial_url
                ...
                try:
                    ...
                    if 1 <= choice <= len(links):
                        selected_text, selected_url = links[choice - 1]
